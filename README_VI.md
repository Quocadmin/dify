...
Dify là gì?
Dify là một nền tảng mã nguồn mở giúp bạn xây dựng các ứng dụng sử dụng trí tuệ nhân tạo (AI) – cụ thể là các mô hình ngôn ngữ lớn (LLM) như GPT, Llama, Mistral... Bạn có thể nhanh chóng tạo ra các ứng dụng AI như chatbot, phân tích tài liệu, hệ thống hỏi đáp, v.v. mà không cần phải biết lập trình phức tạp.

Nền tảng này có giao diện trực quan (kéo thả) để thiết kế các workflow (luồng làm việc) AI, tích hợp nhiều tính năng mạnh mẽ như quản lý mô hình, quan sát/log, tích hợp API và nhiều công cụ hỗ trợ cho agent AI.

Ai nên dùng Dify?
Người mới bắt đầu muốn thử nghiệm với AI.

Doanh nghiệp/Startup muốn xây dựng nhanh các sản phẩm AI, hoặc tích hợp AI vào quy trình.

Nhà phát triển muốn kiểm soát sâu hơn, tự host hoặc chỉnh sửa mã nguồn.

Các tính năng nổi bật
Workflow Canvas:
Cho phép bạn thiết kế, kéo thả các bước trong quy trình AI – như nhập dữ liệu, xử lý với AI, trả kết quả.

Hỗ trợ nhiều mô hình AI:
Không chỉ OpenAI GPT, mà còn nhiều mô hình nguồn mở khác, tích hợp dễ dàng (xem danh sách đầy đủ trên tài liệu Dify).

Prompt IDE:
Giao diện viết, test và so sánh prompt (lệnh cho AI) cực tiện, có thể thử nghiệm nhanh và so sánh hiệu quả các mô hình.

RAG Pipeline:
Hỗ trợ xây dựng các hệ thống “truy xuất kiến thức” (RAG), ví dụ như trích xuất thông tin từ PDF, tài liệu, rồi AI trả lời dựa trên đó.

AI Agent:
Dify cho phép bạn xây dựng agent (AI đa năng) có thể dùng nhiều công cụ (Google Search, DALL·E, WolframAlpha…), hoặc tùy chỉnh theo ý bạn.

LLMOps:
Quản lý, giám sát log, hiệu suất ứng dụng AI, cải thiện prompt và mô hình dựa trên dữ liệu thực tế.

Backend-as-a-Service:
Dify cung cấp sẵn API để bạn tích hợp ứng dụng AI vào hệ thống, website hoặc app riêng.

So sánh với các nền tảng khác
LangChain, Flowise: Nặng về code Python, còn Dify ưu tiên “kéo thả”, API, thân thiện hơn cho người không chuyên code.

OpenAI Assistants API: Chỉ dùng được mô hình OpenAI, còn Dify hỗ trợ rất nhiều nguồn khác nhau.

Dify hỗ trợ triển khai local (tự host trên server riêng), hoặc dùng bản cloud miễn phí.

Cài đặt và sử dụng
Cách nhanh nhất để thử:
Dùng bản Dify Cloud miễn phí tại https://cloud.dify.ai.

Nếu muốn tự cài trên server riêng (tự host):

Cài đặt Docker và Docker Compose trên máy.

Clone repo này về, chuyển vào thư mục docker.

Chạy các lệnh (theo README):
...
cp .env.example .env
docker compose up -d
Truy cập http://localhost/install để hoàn tất cài đặt.

Khi gặp vấn đề?
Xem FAQ.

Hỏi cộng đồng qua Discord, Reddit, hoặc GitHub Discussion.

Tài liệu và cộng đồng
Tài liệu chính thức

Blog hướng dẫn và demo

Group Discord, Reddit, Twitter

Tổng kết nhanh
Dify là nền tảng giúp mọi người – kể cả không chuyên lập trình – có thể tạo ra ứng dụng AI cực nhanh, dễ triển khai và tích hợp đa dạng mô hình AI.
Bạn chỉ cần kéo thả, tùy chỉnh một chút là có ngay sản phẩm AI cho riêng mình!
